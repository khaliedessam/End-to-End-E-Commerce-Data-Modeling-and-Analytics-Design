...
Script Purpose:
    Loads raw data from external CSV files into the Bronze layer as the first
    stage of the ETL pipeline. The procedure refreshes all Bronze tables and
    logs the execution status of each file load for monitoring and auditing.

Actions Performed:
    - Receives a pipeline Run ID for ETL tracking.
    - Creates a log entry before loading each source file.
    - Truncates the target Bronze table.
    - Loads data from CSV files using BULK INSERT.
    - Captures the number of rows loaded.
    - Updates the file load log with:
        * End time
        * Rows loaded
        * Execution status (Success, Warning, or Failed)
        * Error message (if any)
    - Uses TRY...CATCH blocks to handle errors and stop execution when a load fails.

Parameters:
    @run_id UNIQUEIDENTIFIER
        Unique identifier generated by the Master ETL procedure to track
        all file loads belonging to the same pipeline execution.
...
Usage Example:
    ...
    EXEC bronze.load_bronze @run_id;
